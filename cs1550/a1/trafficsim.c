...
int main(int argc, char* argv[])
{
	...
	struct cs1550_sem *n_full, *s_full, *n_empty, *s_empty, *mutex;
	...
	//allocate space for semaphores by casting to struct cs1550_sem. Space for 5 semaphores is needed
	struct cs1550_sem *sem_ptr = (struct cs1550_sem*) mmap(NULL, sizeof(struct cs1550_sem)*5, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, 0, 0);
	...
	*s_con = 0;
	...
	if(fork() == 0){
		...
		while(1){
			...
			sleep(20);	//sleep 20 if the value of rand % 10 is greated than 8
		}
	}
